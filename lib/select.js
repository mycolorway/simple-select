...
Select = (function(superClass) {
  ...
  Select.prototype._init = function() {
    ...
    this.locales = this.opts.locales || Select.locales;
    this._render();
    return this._bind();
  };
  ...
  Select.prototype._render = function() {
    ...
    if (this.opts.allowInput) {
      this.input = $(this.opts.allowInput);
    } else if (this.opts.wordWrap) {
  ...
  Select.prototype._checkBlankOption = function() {
    ...
    placeholder = (this._allowEmpty && $blankOption.text()) || this.opts.placeholder;
    if (placeholder) {
  ...
  Select.prototype._generateList = function(items) {
    ...
    if ($.isPlainObject(items) && !$.isEmptyObject(items)) {
      ...
      $noResults = $('<div class="no-results"></div>').text(this.locales.no_results);
  ...
  Select.prototype._validateInput = function() {
    ...
    if (this.opts.allowInput) {
      ...
      $item = this.list.find(".select-item[data-value='" + value + "']");
      if (!($item.length > 0)) {
      ...
      if (value) {
        ...
        return $.ajax({
          ...
          return function(items) {
            ...
            return _this.list.find('.select-item:first').addClass('selected');
          };
        })(this));
      } else {
        return this._toggleList(false);
  ...
  Select.prototype._focus = function(e) {
    this._focused = true;
    if (this.opts.url && !this.input.val()) {
      return;
    }
    this._toggleList(true);
  ...
  Select.prototype.destroy = function() {
    if (this.opts.allowInput) {
      this.input.off('.simple-select').removeClass('select-result').removeAttr('autocomplete placeholder').insertAfter(this.el);
    }
    this.wrapper.remove();
