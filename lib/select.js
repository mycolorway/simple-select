(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define('simple-select', ["jquery","simple-module","simple-util"], function ($, SimpleModule, simpleUtil) {
      return (root['select'] = factory($, SimpleModule, simpleUtil));
    });
...
Select = (function(superClass) {
  ...
  Select.prototype.setItems = function(items) {
    var $itemEl, item, j, len, results1;
    if (!$.isArray(items)) {
      return;
    }
    this.items = items;
    ...
    for (j = 0, len = items.length; j < len; j++) {
      ...
      if ($.isFunction(this.opts.onItemRender)) {
        results1.push(this.opts.onItemRender.call(this, $itemEl, item));
      } else {
        results1.push(void 0);
      }
    }
